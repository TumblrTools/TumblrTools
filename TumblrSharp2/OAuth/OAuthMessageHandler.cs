namespace TumblrSharp2.OAuth
{
    ...
    internal class OAuthMessageHandler : DelegatingHandler
	{
		...
		public OAuthMessageHandler(IHashProvider hashProvider, string consumerKey, string consumerSecret, Token oAuthToken)
            : this(new HttpClientHandler(), hashProvider, consumerKey, consumerSecret, oAuthToken)
		{ }

		public OAuthMessageHandler(HttpMessageHandler innerHandler, IHashProvider hashProvider, string consumerKey, string consumerSecret, Token oAuthToken)
			: base(innerHandler)
		{
		...
		protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
		{
			...
			if (requestParameters.FirstOrDefault(c => c.Name == "api_key") == null)
			{
				...
				if (this.oAuthToken != null) authorizationHeaderParameters.Add("oauth_token", this.oAuthToken.Key);
				authorizationHeaderParameters.Add("oauth_consumer_key", this.consumerKey);
				...
				string signatureHash = this.hashProvider.ComputeHash(this.consumerSecret, (this.oAuthToken != null) ? this.oAuthToken.Secret : null, signatureBaseString);
