namespace TumblrSharp2.OAuth
{
	...
	internal class OAuthClient : IOAuthClient
    {
		...
		/// </list>
		/// </exception>
		public OAuthClient(IHashProvider hashProvider, string consumerKey, string consumerSecret)
		{
		...
		public async Task<Token> PerformXAuthAsync(string userName, string password)
		{
			...
			using (var client = new HttpClient(new OAuthMessageHandler(this.hashProvider, this.consumerKey, this.consumerSecret, null)))
			{
		...
		/// </list>
		/// </exception>
		public async Task<Token> GetAccessTokenAsync(Token requestToken, string verifier)
		{
			if (requestToken == null)
				throw new ArgumentNullException("requestToken");
            ...
            using (HttpClient client = new HttpClient(new OAuthMessageHandler(this.hashProvider, this.consumerKey, this.consumerSecret, requestToken)))
            {
                ...
                using (HttpResponseMessage response = await client.SendAsync(request).ConfigureAwait(false))
                {
                    if (!response.IsSuccessStatusCode)
                    {
                        throw new OAuthException(
                            String.Format(
                                "GetAccessTokenAsync failed. Status Code: {0}, Message: {1}",
                                response.StatusCode,
