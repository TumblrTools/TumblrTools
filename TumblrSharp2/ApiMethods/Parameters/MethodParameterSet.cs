	...
	public class MethodParameterSet : SortedSet<IMethodParameter>
	{
		...
		public void Add(string name, long value, long? defaultValue = null)
		{
			...
			if(!defaultValue.HasValue || defaultValue.Value != value)
				this.Add(new StringMethodParameter(name, value));
		}
		...
		public void Add(string name, byte[] value)
		{
			...
			if(value != null)
				this.Add(new BinaryMethodParameter(name, value));
		}

		internal string ToFormUrlEncoded()
		{
			return String.Join("&", this.Where(c => c is StringMethodParameter).Select(c => String.Format("{0}={1}", c.Name, this.Encode(c))));
		}

		internal string ToAuthorizationHeader()
		{
			return String.Join(",", this.Select(c => String.Format("{0}=\"{1}\"", c.Name, this.Encode(c))));
		}
