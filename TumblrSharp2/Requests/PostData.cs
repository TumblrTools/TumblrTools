namespace TumblrSharp2.Requests
{
	...
	public class PostData
	{
		...
		{
			this.parameters = new MethodParameterSet();

			this.State = state;
			this.Tags = (tags != null && tags.FirstOrDefault() != null) ? new List<string>(tags) : new List<string>();
		...
		{
			var result = new MethodParameterSet(this.parameters);

			if (this.State != PostCreationState.Published)
				result.Add("state", this.State.ToString().ToLowerInvariant());

			if (this.Tags != null)
				result.Add("tags", String.Join(",", this.Tags.ToArray()));

			if (!String.IsNullOrEmpty(this.Tweet))
				result.Add("tweet", this.Tweet);

			if (this.Date != null)
				result.Add("date", this.Date.Value.ToUniversalTime().ToString("R"));

			if (this.Format != PostFormat.Html)
				result.Add("format", this.Format.ToString().ToLowerInvariant());

			if (!String.IsNullOrEmpty(this.Slug))
				result.Add("slug", this.Slug);
